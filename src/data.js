...
console.log(typeof movies[0].duration);

function getAllDirectors(moviesArray) {
  const directors = moviesArray.map((elem) => {
...
function scoresAverage(moviesArray) {
  const totalScore = moviesArray.reduce((previousValue, currentValue) => {
    console.log(previousValue, currentValue);
    return console.log(previousValue.score + currentValue.score);
  });
} 
...
function dramaMoviesScore(moviesArray) {
  ...
  if (dramamovies.length === 0) {
    ...
    return console.log(result / dramamovies.length);
  }
  ...
  /*const dramamovies = moviesArray.filter((elem) => {
    let newArr = elem.genre.map((elemento) => {
      if (elemento === "Drama") {
        return elem; 
      }
    });
  ...
  dramamovies.map((elem) => {
    return (counter += elem.score);
  });
  const average = parseFloat((counter / dramamovies.length).toFixed(1));
  return average;
...
function orderAlphabetically(moviesArray) {
  ...
  return console.log(titles.slice(0, 20));
}
