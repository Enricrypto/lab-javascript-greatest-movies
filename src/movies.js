...
function howManyMovies(moviesArray) {
  const Spielbergmovies = moviesArray.filter((elem) => {
    return elem.director === "Steven Spielberg";
  });
  const Spielbergfiltered = Spielbergmovies.filter((elem) => {
    return elem.genre.includes("Drama");
  });
  return Spielbergfiltered.length;
}
...
function orderByYear(moviesArray) {
  ...
  const sortbyYear = newArray.sort((a, b) => {
    if (a.year === b.year) {
